 ...
 * 2 cases:
     ...
     * 2:
     *
     * Tney also need to nest. How about maintaining a stack?
     *
     * push if1
     * pop if1, push else1
     * push if2
     ...
     * pop else1
     *
     * if we pop an else then increment the jump target number by one.
     *
     * What if we just increment the counter each time we encountar an if or an else. Nope. Doesn't work. However if we cache the last seen number on a stack it does work. if -> inc & push, else inc & pop & push, endif pop
 *)
